	...
	while (!to_path.empty())
	{
		...
		if (current_unit.unit != nullptr) 
		{
			if (current_unit.unit->GetAction() != DIE || current_unit.unit->GetAction() != DISAPPEAR)
			{
				...
				{
					current_unit.unit->AddPriorizedAction((Action*)App->action_manager->MoveAction(path, current_unit.unit,current_unit.unit_target), current_unit.task_channel);
				}
				else
				{
					current_unit.unit->AddAction((Action*)App->action_manager->MoveAction(path, current_unit.unit, current_unit.unit_target), current_unit.task_channel);
				}
				}
		}
		unit_it++;
...
void j1Pathfinding::PushPath(Unit * unit, iPoint destination,  bool priority, TASK_CHANNELS task_channel, Unit* unit_target)
{
	ToPath getUnit(unit, destination, priority,task_channel,unit_target);
...
ToPath::ToPath(Unit * getUnit, const iPoint & destination, bool priority, TASK_CHANNELS task_channel, Unit* unit_target) :unit(getUnit), destination(destination), priority(priority),task_channel(task_channel),unit_target(unit_target)
{
}
