...
UNIT_TYPE j1Animator::StrToUnitEnum(const char* str) const
{
	...
	if (strcmp(str, "god_unit") == 0)				return GOD_UNIT;
	return NO_UNIT;
}
...
BUILDING_TYPE j1Animator::StrToBuildingEnum(const char* str) const
{
	...
	if (strcmp(str, "lumber_camp") == 0)	return LUMBER_CAMP;
	if (strcmp(str, "mining_camp") == 0)	return MINING_CAMP;
	if (strcmp(str, "town_center_c") == 0)	return TOWN_CENTER_C;
	if (strcmp(str, "barrack_c") == 0)		return BARRACK_C;
	if (strcmp(str, "archery_range_c") == 0)return ARCHERY_RANGE_C;
	if (strcmp(str, "blacksmith_c") == 0)	return BLACKSMITH_C;
	if (strcmp(str, "stable_c") == 0)		return STABLE_C;
	if (strcmp(str, "houseAI") == 0)		return HOUSE_AI;
	if (strcmp(str, "houseBI") == 0)		return HOUSE_BI;
	if (strcmp(str, "houseCI") == 0)		return HOUSE_CI;
	if (strcmp(str, "monastery") == 0)		return MONASTERY;
	if (strcmp(str, "university_c") == 0)	return UNIVERSITY_C;
	if (strcmp(str, "university_i") == 0)	return UNIVERSITY_I;
	if (strcmp(str, "castle") == 0)			return CASTLE;
